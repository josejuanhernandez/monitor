...
public class Model {
	...
	private Model(String name, int lamps, Function<String, String> keyMapper) {
		this.name = name;
		this.lamps = lamps;
		this.goals = isProjector(name) ? goalsOf(name, lamps) : goalsOf(name);
	...
	private static List<Goal> goalsOf(String name, int lamps) {
		List<Goal> list = goalsOf(name);
		for (char suffix : suffixesOf(lamps))
			list.addAll(goalsOf("lamp" + "." + name + "." + suffix));
		return list;
	}

	private static List<Goal> goalsOf(String name) {
		List<String> tags = tagsOf(name);
		List<Goal> list = new ArrayList<>();
		int index = name.indexOf('.');
		while (index > 0) {
			list.add(new Goal(name.substring(0, index), tags));
			index = name.indexOf('.', index+1);
		}
		list.add(new Goal(name, tags));
		return list;
	}
	...
	public static class Goal {
		...
		public Goal(String name, List<String> tags) {
			this.forms = get(name);
			this.tags = tags;
		}
		...
		private Form get(String name, Task task) {
			return getKey(name + "-" + task.name);
		}
		...
		public Form formOf(Task task) {
			return forms[task.ordinal()];
		}
